...
To install this package for local development, first clone this package from github using your favorite git gui or with the command::
    ...
    git clone git@github.com:ddasilva/disco.git
...
We also use `flake8 <https://flake8.pycqa.org/>`__ for linting. To run it locally::
    ...
    flake8 disco --count --select=E9,F63,F7,F82 --show-source --statistics
...
If you now re-import your package (to regenerate ``_version.py``) and print ``disco.__version__`` it should say
``0.2.0``.
